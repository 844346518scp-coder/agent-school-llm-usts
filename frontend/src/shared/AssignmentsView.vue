...
import { api, localToday, reviewLabel, type Assignment, type User, type Submission, type ReviewStatus, type TeachingClass, type Question } from './api'
import MathText from './MathText.vue'
import FeedbackView from './FeedbackView.vue'
import QuestionBank from '../teacher/QuestionBank.vue'
import { assignmentTopic, appendQuestions } from './courseChapters'
const props = defineProps<{ user: User; assignments: Assignment[]; createRequested: boolean; openId?: string; classes: TeachingClass[]; classId: string; loading: boolean; embedded?: boolean }>()
...
const visible = computed(() => props.assignments.filter(a => (!props.classId || a.class_id === props.classId) && a.status === filter.value && `${a.title} ${a.content}`.includes(search.value)))
const choosing = ref(false), pickerError = ref('')
...
const form = ref({ title: '', content: '', topic: '综合练习', due_date: '', class_id: '' })
...
function edit(a?: Assignment) { editId.value = a?.id || ''; form.value = a ? { title: a.title, content: a.content, topic: a.topic || '综合练习', due_date: a.due_date, class_id: a.class_id || '' } : { title: '', content: '', topic: '综合练习', due_date: '', class_id: props.classes.find(c => c.id === props.classId && !c.archived)?.id || '' }; formError.value = ''; choosing.value = false; editing.value = true }
function selectedQuestions(questions: Question[]) {
  try {
    const content = appendQuestions(form.value.content, questions)
    form.value.topic = assignmentTopic(form.value.content, form.value.topic, questions)
    form.value.content = content; choosing.value = false; pickerError.value = ''; ElMessage.success('题目已加入，可继续编辑要求')
  } catch (e) { pickerError.value = (e as Error).message }
}
watch(() => props.createRequested, value => { if (value) edit() }, { immediate: true })
watch(editing, value => { if (!value) { choosing.value = false; pickerError.value = ''; emit('closeCreate') } })
...
async function save(publish = false) {
  ...
  try {
    ...
    filter.value = publish ? 'published' : 'draft'; editing.value = false; if (props.classId && props.classId !== form.value.class_id) emit('filter', form.value.class_id); emit('saved'); ElMessage.success(publish ? '作业已发布至所选班级，接收名单已保存' : '草稿已保存，仅你可见')
...
<template>
  <div class="page-heading" :class="{ 'embedded-heading': embedded }"><div><div v-if="!embedded" class="eyebrow">PRACTICE & REFLECTION</div><h1>{{ teacher ? (classId ? '班级作业' : '全部作业') : '把理解，写进练习里' }}</h1><p>{{ teacher ? '按班级发布任务、查看作答，再把建议交还给学生。' : '留下解题过程，查看老师的建议，再改进一次。' }}</p></div><div class="action-row"><button class="outline-button" :disabled="busy" @click="emit('refresh')"><RefreshCw :size="16"/> 刷新作业</button><button v-if="teacher" class="primary-button" :disabled="busy || !!classes.find(c => c.id === classId)?.archived" @click="edit()"><Plus :size="17"/> 布置作业</button></div></div>
  <div class="panel work-toolbar"><div class="filter-tabs"><button v-if="teacher" :class="{ active: filter === 'draft' }" @click="filter = 'draft'">草稿</button><button :class="{ active: filter === 'published' }" @click="filter = 'published'">已发布</button><button :class="{ active: filter === 'archived' }" @click="filter = 'archived'">已归档</button></div><select v-if="teacher && !embedded" :value="classId" aria-label="按班级筛选作业" :disabled="loading" @change="emit('filter', ($event.target as HTMLSelectElement).value)"><option value="">全部班级</option><option v-for="c in classes" :key="c.id" :value="c.id">{{ c.name }}{{ c.archived ? '（已归档）' : '' }}</option></select><input v-model="search" aria-label="搜索作业" placeholder="搜索标题或题干"/></div>
  ...
  <el-dialog v-model="editing" :title="choosing ? '布置作业 · 从题库选题' : editId ? '编辑作业草稿' : '布置作业'" :width="choosing ? '960px' : '720px'" :close-on-click-modal="false" :close-on-press-escape="!busy" :show-close="!busy"><p v-if="choosing && pickerError" class="form-error" role="alert">{{ pickerError }}</p><QuestionBank v-if="choosing" @selected="selectedQuestions" @cancel="choosing = false; pickerError = ''"/><form v-else class="dialog-form" novalidate @submit.prevent="save(false)"><p class="muted">草稿仅你可见。发布时固定当前班级的有效学生名单；发布后班级与题干锁定。</p><p v-if="editingReadOnly" class="info-banner">所属班级已归档，此草稿只读。可关闭后复制为新草稿并重新选择班级。</p><fieldset class="form-fields" :disabled="busy || editingReadOnly"><label>目标班级<select v-model="form.class_id"><option value="">稍后选择（仅保存草稿）</option><option v-for="c in classes" :key="c.id" :value="c.id" :disabled="c.archived">{{ c.name }} · {{ c.student_count }} 位有效学生{{ c.archived ? '（已归档）' : '' }}</option></select></label><p v-if="!classes.some(c => !c.archived)" class="muted small">请先在“班级管理”中创建班级并添加学生；当前仍可保存未选班级的草稿。</p><label>作业标题<input v-model="form.title" maxlength="100" placeholder="例如：导数定义与几何意义"/></label><label>截止日期<input v-model="form.due_date" type="date" :min="localToday()"/></label><div class="question-source"><div><strong>题目与要求</strong><p>从题库选择已有题目，或直接在下方编写。</p></div><button type="button" class="outline-button" @click="pickerError = ''; choosing = true">从题库选题</button></div><label><span class="sr-only">题目与要求</span><textarea v-model="form.content" rows="7" maxlength="3000" placeholder="支持 $公式$ 与 $$独立公式$$"></textarea></label><details><summary>题干预览</summary><MathText :text="form.content"/></details></fieldset><p v-if="formError" class="form-error" role="alert">{{ formError }}</p><div class="dialog-actions"><button type="button" class="outline-button" :disabled="busy" @click="editing = false">取消</button><button class="outline-button" :disabled="busy || editingReadOnly">保存草稿</button><button type="button" class="primary-button" :disabled="busy || editingReadOnly" @click="save(true)">{{ busy ? '保存中…' : '保存并发布' }}</button></div></form></el-dialog>
