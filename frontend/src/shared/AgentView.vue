...
import { ArrowUp, Sparkles, Plus, Bookmark, Check, Lightbulb, Sigma, BookOpen, History, RefreshCw, Camera } from 'lucide-vue-next'
...
<template>
  ...
  <div class="agent-layout">
    <section class="chat-panel panel">
      <header class="chat-header"><div class="agent-avatar"><Sparkles :size="20"/></div><div><strong>数伴 AI</strong><small>耐心听你说，陪你一步步探索</small></div><div class="chat-header-actions"><button class="subtle-link" :disabled="locked" @click="photoOpen = true"><Camera :size="15"/> 拍照搜题</button><button class="subtle-link" :disabled="locked" @click="historyOpen = true"><History :size="15"/> 对话记录</button><button class="subtle-link" :disabled="locked" @click="newQuestion"><Plus :size="15"/> 新问题</button></div></header>
    ...
    <aside class="agent-aside"><div class="panel side-tip"><div class="section-kicker"><Lightbulb :size="18"/> 好问题的小提示</div><h3>让思考更进一步</h3><p>告诉数伴你正在学什么、尝试过什么，以及具体卡在哪里。</p><div class="quote">“我知道导数的公式，<br>但为什么要用极限定义它？”</div></div><div class="panel"><div class="section-heading"><h3>最近的问题</h3><button class="subtle-link" :disabled="locked" @click="historyOpen = true">全部记录</button></div><button v-for="r in records.slice(0, 5)" :key="r.id" class="recent-question" :disabled="locked" @click="openHistoryRecord(r.id)"><span>{{ r.question }}</span><small>{{ formatDate(r.created_at) }}</small></button><p v-if="!records.length" class="empty-small">还没有提问，从左边开始吧。</p></div><div class="coming-note"><span>接下来，我们还会…</span><p>步骤反馈 · 语音</p><small>拍照搜题已可在对话上方使用，识别文本确认后带入提问；课程引用已可随问答查看</small></div></aside>
