...
import { ArrowUp, Sparkles, Plus, Bookmark, Check, MessageSquare, Lightbulb, Sigma, BookOpen } from 'lucide-vue-next'
...
<template>
  <div class="page-heading"><div><div class="eyebrow">THINK TOGETHER</div><h1>{{ user.role === 'teacher' ? '教学灵感，从对话开始' : '把困惑说出来，一起想明白' }}</h1><p>数伴智能体 · 你的高数{{ user.role === 'teacher' ? '教学' : '学习' }}伙伴</p></div><span class="status-pill"><span class="tiny-dot"/> 演示模式</span></div>
  <div class="agent-layout">
    <section class="chat-panel panel">
      <header class="chat-header"><div class="agent-avatar"><Sparkles :size="20"/></div><div><strong>数伴 AI</strong><small>耐心听你说，陪你一步步探索</small></div><button class="subtle-link" @click="newQuestion"><Plus :size="15"/> 新问题</button></header>
      <div class="chat-body" aria-live="polite">
        ...
        <template v-else><div class="user-message"><span class="message-label">{{ user.name }} · {{ formatDate(active.created_at) }}</span><div>{{ active.question }}</div></div><div class="agent-message"><div class="agent-avatar"><Sparkles :size="18"/></div><div class="answer-content"><div class="message-label">数伴 AI <span class="demo-label">预设演示回复</span></div><MathText :text="active.answer"/><button class="subtle-link save-answer" @click="toggleFavorite"><Check v-if="active.favorite" :size="15"/><Bookmark v-else :size="15"/>{{ active.favorite ? '已收藏到复习本' : '收藏到复习本' }}</button></div></div></template>
        <div v-if="busy" class="thinking">正在读取演示反馈<span>···</span></div>
      </div>
      <form class="chat-compose" @submit.prevent="send"><div class="compose-topic"><span class="tiny-dot"/><label for="topic-select">本次主题</label><select id="topic-select" v-model="topic"><option>函数与极限</option><option>导数与微分</option><option>费曼练习</option><option v-if="user.role === 'teacher'">教学设计</option></select><span class="char-count">{{ question.length }}/2000</span></div><textarea ref="input" v-model="question" maxlength="2000" aria-label="向数伴提问" placeholder="写下你的问题或思考，让我们一起探索…" @keydown.enter.exact.prevent="!$event.isComposing && send()"></textarea><div class="compose-bottom"><span>Enter 发送 · Shift + Enter 换行</span><button class="send-button" type="submit" aria-label="发送问题" :disabled="busy || !question.trim()"><ArrowUp :size="20"/></button></div><p v-if="error" class="form-error" role="alert">{{ error }} 问题已保留，可重新发送。</p></form>
      <p class="chat-disclaimer">当前回复为固定示例，未接入真实模型；请结合教材核对。</p>
    </section>
    <aside class="agent-aside"><div class="panel side-tip"><div class="section-kicker"><Lightbulb :size="18"/> 好问题的小提示</div><h3>让思考更进一步</h3><p>告诉数伴你正在学什么、尝试过什么，以及具体卡在哪里。</p><div class="quote">“我知道导数的公式，<br>但为什么要用极限定义它？”</div></div><div class="panel"><div class="section-heading"><h3>最近的问题</h3><MessageSquare :size="17"/></div><button v-for="r in records.slice(0, 5)" :key="r.id" class="recent-question" @click="emit('select', r.id)"><span>{{ r.question }}</span><small>{{ formatDate(r.created_at) }}</small></button><p v-if="!records.length" class="empty-small">还没有提问，从左边开始吧。</p></div><div class="coming-note"><span>接下来，我们还会…</span><p>拍照识题 · 课程引用 · 分层提示</p><small>这些能力正在规划中</small></div></aside>
  </div>
</template>
