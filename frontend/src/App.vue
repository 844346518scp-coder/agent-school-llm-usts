...
import { api, ApiError, type User, type Conversation, type Assignment } from './shared/api'
...
const navigation = computed(() => teacher.value ? [
  ...
  { id: 'assignments', label: '作业管理', icon: ClipboardList }, { id: 'students', label: '我的班级', icon: Users }, { id: 'records', label: '对话记录', icon: History },
] : [
...
async function loadData() {
  ...
  try {
    const [r, a] = await Promise.all([api<Conversation[]>('/conversations'), api<Assignment[]>('/assignments')])
    if (current !== generation || !user.value) return
    records.value = r; assignments.value = a; loadError.value = ''
  } catch (e) { if (current === generation && user.value) loadError.value = (e as Error).message }
}
function navigate(id: string) { page.value = id; mobileNav.value = false; if (id !== 'agent') { initialQuestion.value = ''; selectedId.value = '' } }
...
function reset() { generation++; user.value = null; records.value = []; assignments.value = []; page.value = 'dashboard'; initialQuestion.value = ''; selectedId.value = ''; mobileNav.value = false; createRequested.value = false; loadError.value = '' }
...
<template>
  ...
  <div v-else class="app-shell" :class="{ 'teacher-theme': teacher }">
    ...
    <div class="app-body"><header class="topbar"><button class="icon-button mobile-only" aria-label="打开导航" @click="mobileNav = true"><Menu :size="22"/></button><div class="breadcrumb"><span>工作台</span><ChevronRight :size="13"/><strong>{{ navigation.find(n => n.id === page)?.label }}</strong></div><div class="topbar-right"><span class="local-indicator"><span class="tiny-dot"/> 本地体验版</span><span class="topbar-divider"></span><GraduationCap :size="18"/><span>高等数学</span></div></header><main class="main-content"><div v-if="loadError" class="load-error" role="alert"><span>{{ loadError }}</span><button class="subtle-link" @click="loadData"><RefreshCw :size="14"/> 重新加载</button></div>
      ...
      <TeacherHome v-else-if="page === 'dashboard'" :user="user" :records="records" :assignments="assignments" @navigate="navigate" @create="create"/>
      ...
      <AssignmentsView v-else-if="page === 'assignments'" :user="user" :assignments="assignments" :create-requested="createRequested" @saved="loadData" @close-create="createRequested = false"/>
      ...
      <template v-else-if="page === 'students' && teacher"><div class="page-heading"><div><div class="eyebrow">GROW TOGETHER</div><h1>看见每一位同学</h1><p>高等数学 · 演示班级，当前只有一名预设学生。</p></div><span class="status-pill">1 位学生</span></div><section class="panel"><div class="section-heading"><h3>班级成员</h3><span class="muted small">账号与作业数据</span></div><div class="class-student"><span class="user-avatar">林</span><div><h3>林同学</h3><p class="muted">student · 学生演示账号</p></div><div><strong>{{ assignments.filter(a => a.submissions?.length).length }} / {{ assignments.length }}</strong><p class="muted">已提交作业</p></div><button class="outline-button" @click="navigate('assignments')">查看作业 <ArrowUpRight :size="15"/></button></div><div class="teaching-reminder"><h4>先观察，再判断</h4><p>本版仅记录作业提交，不推断知识点掌握程度。多班级管理、成员邀请、学情诊断和反馈复核尚未开放。</p></div></section></template>
      <footer class="workspace-footer"><span>数伴 · 让每一步学习都有回应</span><span>MVP 0.1 <span class="tiny-dot"/> 持续生长中</span></footer>
    </main></div>
    <el-dialog v-model="help" title="欢迎体验数伴" width="520px"><div class="help-content"><p>这是一个高数教学与学习的最小可行版本。</p><p><strong>已经可以体验：</strong>学生 / 教师登录、记住登录、课程入口、演示问答与收藏、历史记录、教师发布作业、学生提交、教师查看作答。</p><p><strong>当前尚未接入：</strong>真实 AI、拍照识题、语音、RAG、自动评分和多班级管理。页面会明确标注演示内容。</p><p>所有预设账号均用于本地体验，不要填入真实师生隐私。账号切换请先退出登录；退出会撤销已记住的会话。</p></div></el-dialog>
  </div>
</template>
