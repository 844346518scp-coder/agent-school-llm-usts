...
const classWorkspaceKey = ref(0)
const workspaceEntry = ref<{ classId: string; create: boolean } | null>(null)
...
const navigation = computed(() => user.value?.must_change_password ? [{ id: 'account', label: '设置密码', icon: Settings }] : [...(teacher.value ? [
  { id: 'dashboard', label: '教学概览', icon: LayoutDashboard }, { id: 'students', label: '班级管理', icon: Users },
...
async function loadData() {
  ...
  try {
    const [r, a, stats, c] = await Promise.all([api<Conversation[]>('/conversations'), api<Assignment[]>('/assignments'), teacher.value ? api<TeachingStats>('/teaching/stats' + query) : Promise.resolve(null), teacher.value ? api<TeachingClass[]>('/classes') : Promise.resolve([])])
...
function create() { if (teacher.value) { navigate('students'); workspaceEntry.value = { classId: classFilter.value, create: true } } else { createRequested.value = true; navigate('assignments') } }
function reset() { generation++; workspaceEntry.value = null; teachingStats.value = emptyStats(); classes.value = []; classFilter.value = ''; dataLoading.value = false; user.value = null; records.value = []; assignments.value = []; page.value = 'dashboard'; initialQuestion.value = ''; selectedId.value = ''; mobileNav.value = false; createRequested.value = false; loadError.value = '' }
...
<template>
  ...
  <div v-else class="app-shell" :class="{ 'teacher-theme': teacher }">
    ...
    <div class="app-body"><header class="topbar"><button class="icon-button mobile-only" aria-label="打开导航" @click="mobileNav = true"><Menu :size="22"/></button><div class="breadcrumb"><span>工作台</span><ChevronRight :size="13"/><strong>{{ navigation.find(n => n.id === page)?.label }}</strong></div><div class="topbar-right"><span class="local-indicator"><span class="tiny-dot"/> 本地教学空间</span><span class="topbar-divider"></span><GraduationCap :size="18"/><span>高等数学</span></div></header><main class="main-content"><div v-if="loadError" class="load-error" role="alert"><span>{{ loadError }}</span><button class="subtle-link" @click="loadData"><RefreshCw :size="14"/> 重新加载</button></div>
      ...
      <AssignmentsView v-else-if="page === 'assignments' && !teacher" :user="user" :assignments="assignments" :create-requested="createRequested" :classes="classes" :class-id="classFilter" :loading="dataLoading" @filter="filterClass" @refresh="loadData" @saved="loadData" @close-create="createRequested = false"/>
