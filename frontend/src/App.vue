...
<template>
  ...
  <div v-else class="app-shell" :class="{ 'teacher-theme': teacher }">
    ...
    <el-dialog v-model="help" title="欢迎体验数伴" width="520px"><div class="help-content"><p>这是一个高数教学与学习的最小可行版本。</p><p><strong>已经可以体验：</strong>学生 / 教师登录、记住登录、课程入口、模型 / 演示问答与课程引用、收藏、历史记录、教师题库、草稿与发布、学生提交、教师人工反馈和历史记录、作业归档。</p><p><strong>能力边界：</strong>已接入模型兼容接口与本地课程检索；未配置模型时仍为演示。拍照识题、步骤反馈和诊断已有后端接口，页面入口待接入；语音、自动评分、多班级和向量检索尚未实现。</p><p>所有预设账号均用于本地体验，不要填入真实师生隐私。账号切换请先退出登录；退出会撤销已记住的会话。</p></div></el-dialog>
  </div>
</template>
