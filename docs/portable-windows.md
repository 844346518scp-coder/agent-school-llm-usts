...
教师工作空间v0.3/schema v3已以8799cd0上传原C分支；其后的9/20至21教师入口、成员批量操作与章节分组选题仍在本地。本轮没有重建便携ZIP，旧包不含这些功能。下列“演示账号”步骤只适用于该历史ZIP；若用当前源码新打包，空库默认教师建档，演示种子须显式启用，并必须重新执行隔离验收，不能复用旧包结果。
