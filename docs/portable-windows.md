> 最后更新：2026-09-19。当前默认方案已改为“普通源码下载后自动初始化”，见[source-startup.md](source-startup.md)。本文记录可选便携工具及2026-09-18生成的历史C版ZIP；旧ZIP不含B/C集成，不是当前交付，也不得用来判断main的功能状态。

本地教师工作空间v0.3/schema v3是此后的未发布增量，本轮没有重建便携ZIP。下列“演示账号”步骤只适用于该历史ZIP；若用当前源码新打包，空库默认教师建档，演示种子须显式启用，并必须重新执行隔离验收，不能复用旧包结果。
