...
- 学习辅助（纯前端，2026-09-18）：复习收藏页按对话 topic 实时生成标签筛选（数量、仅收藏态），「同标签重练」只做客户端跳转——预填同知识点练习请求文本并预选主题，是否发送由学生决定，发送仍走既有 POST /api/conversations，无新接口；倒计时专注模式为 FocusTimer.vue 本地组件（预设/自定义时长、Date.now 截止时间计时、暂停继续、关闭弹窗后头部胶囊续显、完成轮数统计、WebAudio 提示），不发请求、不写库、不持久化，离开智能体页或刷新即重置。两者都不改变后端、数据库与接口约定。
...
| 2026-09-18 | 同标签重练与专注计时只做纯前端，不新增接口与存储 | 复用既有收藏记录的 topic 标签与 POST /conversations；专注计时为本地内存状态；浏览器端到端已验证，离开页面即重置 |
