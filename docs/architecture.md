...
用户确认先完成本地题库、作业状态、人工反馈和统计，正式账号/多班级/PostgreSQL/云端部署后置。仅在现有目录新增普通文件，不改目录结构；没有引入依赖或外部模型。前端进入作业页/概览/演示班级、操作完成或手动刷新时读取接口，无实时推送。保留 0.1 上传历史，本轮 0.2 准备提交到独立分支 codex/c-local-teaching，凭据确认后上传；保留9a57238基线，最新main已合并B，本次不做B/C集成。
