...
最后更新：2026-09-21。教师工作空间 v0.3，当前迁移版本号：3。仅支持 SQLite；0.2 B/C集成保持schema v2，本轮账号/班级实现新增v3。PostgreSQL / pgvector仍单独后置。

## 9/20 界面增量与兼容

9/21隔离模拟测试没有新增迁移，完整82项回归包含既有迁移测试且通过；实际库在该回归前后哈希相同。随后双端页面在独立QA库出现SQLite锁冲突，属于待定位的运行期稳定性问题，不以重跑迁移或覆盖数据库处理，详见[测试报告](../docs/simulation-test-report.md)。

班级管理整合作业、成员独立页面及题库章节分组仅改变前端，schema仍为3，没有新增迁移或批量回写。章节沿用现有topic字符串，旧自定义值保留；标准章名前缀仅在前端匹配，编辑保存才按教师选择更新。v0.3及后续界面已上传C分支，本轮与main拍照入口整合，不增加schema版本或修改实际数据库。

9/21新增批量成员操作复用既有成员PATCH逐人写入，不是新迁移或全库批量更新。作业表单移除章节下拉，但topic列保留；新手写/旧空章节回落综合练习，选题时自动推导，未重新选题的旧非空值保留。
