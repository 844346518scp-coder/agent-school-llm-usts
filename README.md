...
截图中的 `Frontend dependencies missing` 表示下载的是源码而缺少开发依赖；只复制启动文件、源码ZIP或开发机的 `.venv` 无法保证换机运行。

测试者使用已构建的 `shuban-windows-x64-日期时间.zip`，完整解压到可写目录后双击 `一键启动.cmd`，默认访问 `http://127.0.0.1:18080/`（不可用时自动选端口，以打开的网址为准）。内含Python 3.11、后端依赖、SQLite及前端静态网页，无需安装Node.js/Python或联网下载依赖。当前适用于Windows 10/11 x64，当前包为C版，B/C尚未集成。普通GitHub源码ZIP不含运行环境。

开发者在本项目环境执行 `.venv/Scripts/python.exe build-portable.py` 生成被忽略的 `dist/` 构建产物。包内不携带本机数据库、环境配置或日志，首次运行创建演示数据。操作、版本升级与限制见 [便携包说明](docs/portable-windows.md)。
