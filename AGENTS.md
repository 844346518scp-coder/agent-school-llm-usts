...
- 收藏重练与专注计时（2026-09-18）：复习收藏页（RecordsView favorites 态，学生侧错题收藏入口）新增按 topic 标签的筛选行（实时数量、仅收藏页显示）与每张卡片的「同标签重练」，以及选中标签后的「重练「xx」」按钮；点击后跳转智能体页，预填同知识点练习题请求文本并预选主题，由学生自行发送（复用 POST /conversations，不自动提问，无新接口）。搜题页（AgentView 对话头）新增 frontend/src/shared/FocusTimer.vue：25/15/10/5 分钟预设与 1–120 分钟自定义、SVG 圆环倒计时、暂停/继续/结束、关闭弹窗后头部胶囊继续显示、归零进入完成态并统计本页累计轮数/分钟、WebAudio 三音提示（失败静默）。纯前端、只存内存、无网络请求，离开本页或刷新即重置（弹窗内有说明）；el-dialog 须 append-to-body 且 close-on-click-modal=false，否则在对话头 flex 容器内因过渡态 pointer-events:none 叠加遮罩点击关闭而卡死。**验证：vue-tsc 与 vite build 通过（2914 modules）；真实前后端浏览器端到端通过（标签筛选/预填/主题预选、计时暂停继续/关闭续显/归零完成统计、拍照弹窗回归、console 无 error）；未做真机移动端、提示音兼容性与跨页持久化。**原型清单中的 localStorage 错题本、迷你测验、语音类功能仍未实现。
