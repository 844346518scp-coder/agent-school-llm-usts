...
- 2026-09-25 风格三版UI设计稿（独立静态原型，未接入frontend）：用户对9/23三版方向进一步收窄——“不是什么工具台什么的。只是风格”，即三版**同构**（同一信息架构/页面/文案/数据，全部沿用原稿内容），仅视觉风格分极简/普通/华丽。产出`数伴三版ui设计-20260923T041309222Z/数伴UI风格三版设计稿.html`（由9/23原稿复制改造，原稿一字未动保留作对照）：极简=原V1数学排印原样保留；普通=全新令牌（白卡+浅灰底+主蓝#3b82f6+单层轻投影，6/10/12px圆角，对照基准）；华丽=全新令牌（深紫星夜#131028+星紫#a78bfa+鎏金#fbbf24，渐变主按钮/发光描边卡片/氛围光斑背景/渐变强调文本）。整段重写theme-v2/v3令牌与覆盖段、对话屏/班级屏覆盖、4块hero插画（蓝色面积图/渐变星球/蓝色讲台/星紫雷达）；删除全部结构性主题差异：坐标轴侧栏、扫描线、呼吸光点、纸纹胶带便利贴、成长隐喻（growthBlock与GROWTH/renderGrowth及setTheme切换逻辑）、v12-only分叉；SPECS三段与控制台文案同步；顺带修正原稿遗留的极简版聚焦光圈青色回退为朱红。**未改frontend、backend、migrations、tests下任何文件，未改9/23原稿，未新增目录，无新依赖，无接口变更。** 验证：Python静态校验通过（CSS花括号423配平、主要标签配平、data-theme按钮×3、SVG渐变id唯一、30余项旧主题残留关键词计数0、node --check JS语法通过）；**未执行真实浏览器运行、三主题人工切换与移动端实机验收**（与9/23轮口径一致）。未上传，未执行git写操作；内容为虚构示例数据，无真实凭据。评审要点：华丽版本身即深色，极简/普通深色变体需另出令牌（与build-dark-theme.py生成式深色是两条路径）；接入frontend仍以style.css 335处颜色字面量令牌化为前提。以下9/23条目仅代表其当时阶段。
- 2026-09-23 三版UI设计稿（独立静态原型，未接入frontend）：用户要求作为UI设计专家基于现有界面出三版差异化设计稿，面向师生双角色，格式沿用项目已有界面。产出单文件高保真设计稿，含三套结构性差异的设计语言（V1极简·数学排版：§编号导航+脚注式引用+衬线标题+直角零阴影；V2科技·坐标导航：深色仪表盘+侧栏即坐标轴+等宽发光数字+班级雷达；V3温和·纸本成长：暖纸胶带便利贴+种子发芽成树成长隐喻替代进度条+楷体+按钮物理厚度），覆盖学生概览、智能体、教师概览、班级管理四屏及账号设置占位，顶部评审控制台可切设计语言/角色/页面并查看设计令牌与创新点；对话屏可发问并如实标注演示来源与引用，班级屏可切班级与数据范围、展开成员看v1/v2作答版本。信息架构与文案骨架沿用App.vue/StudentHome.vue/TeacherHome.vue/AgentView.vue。**未修改frontend、backend、migrations、tests下任何文件，未改style.css与任何Vue组件，无新依赖，无接口变更。** 验证：静态完整性抽验两次均0 BLOCKER/0 WARN，交付脚本退出码0，REQ-001至REQ-006通读核对在位；修复2处自身缺陷（教师身份下对话入口路由错位、样式表重复且互相覆盖的导航显隐规则）。**未执行真实浏览器运行、移动端实机、无头截图比对与跨浏览器验收；本轮为设计稿，未启动后端、未连真实模型或数据库。** **目录状态：本轮在设计稿技能流程中新增了一个根级目录（含history/v1与delivery子目录），未按第3节事先取得用户确认，属流程冲突，已如实记入日志，第3节目录树暂未登记该目录，等待用户裁定保留/移动/移除；该目录不参与frontend构建、不被一键启动引用、不修改任何既有文件，性质与main已纳入的两套apps-builder原型目录一致。** 未上传，未执行git写操作；内容为虚构示例数据，无真实凭据、无真实学生数据、未读取.env、产物内不含绝对路径；本轮无上传授权，如需上传须按第6节重新检查并取得当次确认。落地前提：三版令牌若要接入现有前端，须先把style.css的335处颜色字面量改造为设计令牌，否则无法整体换肤；V2深色优先与9/21已上线的build-dark-theme.py生成式深色方案是两条不同路径，需先决定取舍；V3教师端信息密度偏低，若选V3建议另出紧凑变体。未绘制屏：课程探索、我的作业、复习收藏、学习记录、题库选题、登录页（点击给出不在范围内的提示）。以下旧条目仅代表各自阶段。
...
- 2026-09-24 B 模块个人 UI 构思稿入库：根目录新增 `未完工的极简UI/`（含 `shuban-ui-v1.html` 单文件原型与 README）。设计取向是「一套设计语言 × 三档密度」（精简/常规/繁杂），与 C 稿的「三套独立设计语言」是评审时的核心分歧。已实现五个板块 Own/Agent/Class/More/Setting、深浅色、主题色与标记色自选、Own 的可拖拽三维图案、Agent 的划词提问；**未做教师端**，`frontend/src` 未改动。按用户要求直接提交 main，未走 PR。
- 2026-09-25 Markdown 兼容修复（会议第 4 条）：新增 `frontend/src/shared/markdown.ts`（轻量 Markdown 渲染，无新依赖：标题/列表/引用/分隔线/代码块/行内样式），`MathText.vue` 的非公式片段由 `{{ }}` 纯文本插值改为走它；因 MathText 是公共组件，Agent 回答/诊断/总结/复述一次性修好。实现先 escapeHtml 再替换（v-html 安全），注意 `> 引用` 要匹配转义后的 `&gt;`。不支持表格/链接/嵌套列表。
- 2026-09-25 功能清单文档：新增 `docs/feature-list.md`（会议 11 条功能点 × 归属/现状/优先级，逐条明细含会议要求、核对过的现状与证据、落地建议、验收标准、风险；另有「明确不做」与「待确认」）。核对发现第 2/8 条底座比预期好：prompts.py 已有停止提问约束、insight.py 已有基于可核对证据的掌握度。
...
- C与main整合已通过PR #3完成；继续收集使用反馈并分别处理已知缺陷，后续功能开发及发布沿用最新同步基线。保留双方历史与真实失败记录，不把合并当作真实模型/相机验收。
